...
def generate_final_report(db_manager: DatabaseManager, processing_time: float, dry_run: bool = False):
    ...
    try:
        ...
        print("=" * 60)
        print(f"[FILES] File totali analizzati: {stats['general']['total_files']}")
        ...
        if dry_run:
            ...
            print(f"[SIMULAZIONE] File non supportati: {stats['general']['unsupported_files']}")
        else:
            ...
            print(f"[UNSUPPORTED] File non supportati: {stats['general']['unsupported_files']}")
